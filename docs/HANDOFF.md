...
## Applied-question expansion completed

2026-09-10: User explicitly approved finishing previously unpublished applied-bank and selection changes. Reviewed pending diffs rather than overwriting them. Release has 686 questions: 36 Fundamentals, 26 in every other section; 187 applied/499 foundation; seven multi-select. Adds 166 depth items and versions substantive revisions (20 Fundamentals items and s02-007). Existing saved snapshots remain authoritative for retakes. Section selection represents each selected area before filling a cap; cumulative prioritizes applied items and topic variety. Full forms select 90 applied items with existing domain quotas and all 28 primary objectives. This does not establish complete subtopic coverage or live-exam difficulty equivalence.

All 15 tests passed, covering IDs/stems/citation boundaries, minimum section depth, metadata, revised versions, capped selection, primary-objective coverage, domain quotas, exact multi-select grading, retakes, timers and rolling scores. App syntax passed. Reviewed representative new questions and all seven multi-select items; no claim of independent full-bank content review. Local browser confirmed 686 total, 36/26 section counts, all-section unlock, and full applied form A launching with 90 questions/90-minute timer and dashboard hidden. Live learner progress was not touched.

Updated README/QUESTION_BANK to match final behavior. Kept docs/EXAM_STYLE_BENCHMARK.md local-only via .gitignore because it is reference research, consistent with user instructions. Reference originals and all research authoring materials remain ignored. Remaining product work includes hands-on PBQs, deeper scenario/distractor review, and full subtopic coverage; these are outside this publication task.
